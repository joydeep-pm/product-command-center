...
- Extended the same legibility pass into `Strategy`, `AOP`, and `Risks` in commit `2929f9f`, raising table, card, and row typography without changing data or workflows.
- Verified with pane-specific local screenshots at `/Users/joy/autoresearch/.gstack/design-reports/screenshots/strategy-after-desktop.png`, `/Users/joy/autoresearch/.gstack/design-reports/screenshots/aop-after-desktop.png`, `/Users/joy/autoresearch/.gstack/design-reports/screenshots/risks-after-desktop.png`, plus the matching mobile captures.
...
- Extended the same readability cleanup into `Roadmap` and `Product Deep-Dives` in commit `STYLE_PENDING`, focusing on filter-bar legibility, kanban/deep-dive card copy, and metadata sizing rather than changing workflows.
